...
///// 在旋转后将根置黑、孩子置红，虽需三次染色但效率更高
      ...
      BinNodePosi(T) &tmp = IsRoot(*g) ? this->_root : ( IsLChild(*g) ? (*g).parent->lc : (*g).parent->rc );
      BinNodePosi(T) r = tmp = this->rotateAt ( x ); //调整后的子树根节点
      r->parent = gg; //与原曾祖父联接
...
template <typename T> void RedBlack<T>::solveDoubleBlack ( BinNodePosi(T) r ) {
   ...
   if ( IsBlack ( s ) ) { //兄弟s为黑
      ...
      // 以下，通过旋转重平衡，并将新子树的左、右孩子染黑
         BinNodePosi(T) &tmp = IsRoot(*p) ? this->_root : ( IsLChild(*p) ? (*p).parent->lc : (*p).parent->rc );
         BinNodePosi(T) b = tmp = this->rotateAt ( t ); //旋转
      ...
      BinNodePosi(T) &tmp = IsRoot(*p) ? this->_root : ( IsLChild(*p) ? (*p).parent->lc : (*p).parent->rc );
      this->_hot = p; tmp = this->rotateAt ( t ); //对t及其父亲、祖父做平衡调整
