...
public:
   ...
   //BinNodePosi(T) attachAsLC ( BinNodePosi(T) x, BinTree<U>* &T ); //T作为x左子树接入
   //BinNodePosi(T) attachAsRC ( BinNodePosi(T) x, BinTree<U>* &T ); //T作为x右子树接入
   ...
   //可视化接口
   int updateLevel();//更新层次，返回最大层次
   void updatePosition(int levelMargin, int sibMargin, int margin, int r, int leftMargin);//更新坐标
}; //BinTree
...
template <typename T> int BinTree<T>::updateLevel()
{
    if(this->_root)
        return this->_root->updateLevel();
}

template <typename T> void BinTree<T>::updatePosition(int levelMargin, int sibMargin, int margin, int r, int leftMargin)
{
    if(this->_root)
        this->_root->updatePosition(levelMargin, sibMargin, margin, r, leftMargin);
}
...
int BinTree<T>::remove ( BinNodePosi(T) x ) { //assert: x为二叉树中的合法位置
   BinNodePosi(T) &tmp = IsRoot(*x) ? this->_root : ( IsLChild(*x) ? (*x).parent->lc : (*x).parent->rc );
   tmp = NULL; //切断来自父节点的指针
